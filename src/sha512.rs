...
impl Sha512 {
    fn padding_message(text: String) -> Vec<Vec<u64>> {
        //let pad_size = 512;
        ...
        let mut subs = split_by_length(&bits, 1024);

        let len = subs.len() - 1;
        subs[len].push('1');
        while subs[len].len() != 896 {
            subs[len].push('0');
        }
        let ender = format!("{:0128b}", bits.len());
        subs[len].push_str(ender.as_str());

        let mut bits_pad = Vec::new();

        for el in &subs {
            bits_pad.push(split_by_length(&el, 64));
        }
        ...
        for i in bits_pad {
            ...
            for el in i {
                temp.push(u64::from_str_radix(&el, 2).unwrap());
            }
...
fn delta_0(x: u64) -> u64 {
    x.rotate_right(1) ^ x.rotate_right(8) ^ x >> 7
}
fn delta_1(x: u64) -> u64 {
